...
import com.wrms.newtype.counseling.api.dto.request.AddCommentRequest;
import com.wrms.newtype.counseling.api.dto.request.ResolveCounselingRequest;
import com.wrms.newtype.counseling.api.dto.request.UpdateCounselingStatusRequest;
import com.wrms.newtype.counseling.api.dto.response.CounselingHistoryResponse;
 ...
 * 상담 상세 화면
 */
@Route(value = "counseling-detail/:counselingId", layout = MainLayout.class)
...
public class CounselingDetailView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public CounselingDetailView(CounselingQueryService counselingQueryService,
                                CounselingCommandService counselingCommandService,
        ...
        // Initialize dialogs
        transferDialog = new TransferDialog(counselingCommandService, userQueryService, this::refreshData);
        escalationDialog = new EscalationDialog(counselingCommandService, this::refreshData);
    }

    @Override
    public void beforeEnter(BeforeEnterEvent event) {
        Optional<String> idParam = event.getRouteParameters().get("counselingId");
    ...
    private void loadCounselingData() {
        try {
            counseling = counselingQueryService.findById(counselingId);
            refreshUI();
        } catch (Exception e) {
    ...
    private void refreshUI() {
        titleHeader.setText("상담 #" + counseling.id());
    ...
    private void buildInfoSection() {
        ...
        row1.setWidthFull();
        row1.add(
            createInfoItem("고객명", counseling.customerName()),
            createInfoItem("연락처", counseling.contact()),
            createInfoItem("상담유형", counseling.counselingType())
        );
        ...
        row2.setWidthFull();
        row2.add(
            createInfoItem("상태", getStatusBadge(counseling.status())),
            createInfoItem("우선순위", getPriorityBadge(counseling.priority())),
        ...
        row3.setWidthFull();
        row3.add(
            createInfoItem("현재 담당자", getCounselorName(counseling.counselorId())),
            createInfoItem("에스컬레이션 레벨", String.valueOf(counseling.escalationLevel())),
            createInfoItem("이관 횟수", String.valueOf(counseling.transferCount()))
        );
        ...
        Paragraph contentParagraph = new Paragraph(counseling.content());
    ...
    private Span getPriorityBadge(Priority priority) {
    ...
    private String formatSlaDate(CounselingResponse counseling) {
    ...
    private HorizontalLayout createActionButtons() {
        ...
        statusComboBox.setValue(counseling.status());
        ...
        resolveButton.setEnabled(
            ...
            counseling.status() == CounselingStatus.ESCALATED
        );
    ...
    private void changeStatus(CounselingStatus newStatus) {
        try {
            Long currentUserId = counseling.counselorId();
            UpdateCounselingStatusRequest request = new UpdateCounselingStatusRequest(newStatus, currentUserId);
            counselingCommandService.updateStatus(counseling.id(), request);
    ...
    private void resolveCounseling() {
        try {
            Long currentUserId = counseling.counselorId();
            ResolveCounselingRequest request = new ResolveCounselingRequest("해결 완료", currentUserId);
            counselingCommandService.resolve(counseling.id(), request);
    ...
    private void buildHistorySection() {
        ...
        List<CounselingHistoryResponse> histories = counselingQueryService.findHistoryByCounselingId(counselingId);
        ...
        for (CounselingHistoryResponse history : histories) {
            ...
            String timestamp = history.performedAt().format(formatter);
            String performer = getUserName(history.performedBy());
            String action = history.actionType() != null ? history.actionType().getDescription() : "알 수 없음";
            ...
            if (history.comment() != null && !history.comment().isEmpty()) {
                Paragraph comment = new Paragraph(history.comment());
    ...
    private void addComment(String comment, TextArea commentArea) {
        ...
        try {
            Long currentUserId = counseling.counselorId();
            AddCommentRequest request = new AddCommentRequest(comment, currentUserId);
            counselingCommandService.addComment(counseling.id(), request);
    ...
    private String getUserName(Long userId) {
        ...
        Optional<UserResponse> user = userQueryService.findById(userId);
        return user.map(UserResponse::name).orElse("알 수 없음");
    }
}
