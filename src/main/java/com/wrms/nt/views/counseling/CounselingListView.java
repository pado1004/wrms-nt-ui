...
public class CounselingListView extends VerticalLayout {
    ...
    public CounselingListView(CounselingService counselingService, UserService userService) {
        ...
        // Initialize dialog
        formDialog = new CounselingFormDialog(counselingService, this::updateList);
    }
    ...
    private void configureGrid() {
        ...
        // ID
        grid.addColumn(Counseling::getId)
            .setHeader("ID")
            .setAutoWidth(true)
            .setFlexGrow(0);

        // 고객명
        grid.addColumn(Counseling::getCustomerName)
            .setHeader("고객명")
            .setAutoWidth(true);

        // 상담유형
        grid.addColumn(Counseling::getCounselingType)
            .setHeader("상담유형")
            .setAutoWidth(true);

        // 상태 (Badge)
        grid.addColumn(new ComponentRenderer<>(counseling -> {
            Span badge = new Span(counseling.getStatusEnum().getDescription());
            badge.getElement().getThemeList().add("badge");

            switch (counseling.getStatusEnum()) {
                case ESCALATED:
        ...
        grid.addColumn(new ComponentRenderer<>(counseling -> {
            Priority priority = counseling.getPriorityEnum();
        ...
        // 담당자
        grid.addColumn(counseling -> getCounselorName(counseling.getCounselorId()))
            .setHeader("담당자")
            .setAutoWidth(true);
        ...
        grid.addColumn(new ComponentRenderer<>(counseling -> {
            ...
            viewButton.addClickListener(e ->
                getUI().ifPresent(ui -> ui.navigate(CounselingDetailView.class, counseling.getId()))
            );
            return viewButton;
    ...
    private void updateList() {
        List<Counseling> counselings = counselingService.findAll();

        // 검색 필터
        String searchTerm = searchField.getValue();
        if (searchTerm != null && !searchTerm.isEmpty()) {
            counselings = counselingService.searchByCustomerName(searchTerm);
        }
        ...
        if (statusValue != null && !"전체".equals(statusValue)) {
            ...
            if (status != null) {
                counselings = counselings.stream()
                    .filter(c -> c.getStatusEnum() == status)
                    .toList();
            }
        }
        ...
        if (priorityValue != null && !"전체".equals(priorityValue)) {
            ...
            if (priority != null) {
                counselings = counselings.stream()
                    .filter(c -> c.getPriorityEnum() == priority)
                    .toList();
            }
        }
